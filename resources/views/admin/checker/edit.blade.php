...
@section('content')
    ...
    <div class="container-xl px-4 mt-n10">
        <!-- Style Reference-->
        <form class="row" id="banner" action="{{ route('admin.checker.update', $data->id) }}" method="POST"
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-8 mb-3">
                                    <select class="form-select @error('jenis') is-invalid @enderror" id="jenis"
                                        ...
                                        <option value="Rumah" {{ $data->jenis == 'Rumah' ? 'selected' : '' }}>Rumah
                                        </option>
                                        <option value="Kantor" {{ $data->jenis == 'Kantor' ? 'selected' : '' }}>Kantor
                                        </option>
                                        <option value="Apartemen" {{ $data->jenis == 'Apartemen' ? 'selected' : '' }}>
                                            Apartemen</option>
                                    </select>
                                    @error('jenis')
                                ...
                                <div class="col-sm-8 mb-3">
                                    <input type="text" class="form-control @error('kategori') is-invalid @enderror"
                                        id="kategori" name="kategori" value="{{ $data->kategori }}" required>
                                    @error('kategori')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="col-sm-8 mb-3">
                                    <select class="form-select @error('jenis') is-invalid @enderror" id="kualitas"
                                        ...
                                        @foreach ($quality as $row)
                                            <option value="{{ $row->id }}"
                                                {{ $data->id_kualitas == $row->id ? 'selected' : '' }}>{{ $row->nama }}
                                            </option>
                                        @endforeach
                                    </select>
                                    @error('jenis')
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Set harga on first load if value exists
            if (kualitasSelect.value) {
                hargaInput.value = kualitasData[kualitasSelect.value] ?? '';
            }
